...
(** (If you encounter warning messages when importing these files, don't worry:
    ...
    The next commands turn on options that make programming in Coq a bit easier;
    it is usually a good idea to include those at the beginning of your
    files. The first two allow us to omit arguments that can be automatically
    inferred by Coq (usually, type parameters for polymorphic functions); the
    last one instructs Coq's printer to omit these arguments when printing
    terms. If this sounds a bit mysterious to you, don't worry -- you don't need
    to understand what they do right now. We will come back to them later. *)
...
(** * Defining simple data types
    ...
    In this chapter, we will study how to use Coq to implement and verify
    red-black-tree operations for building an abstract data type of sets. We
    will begin by showing how to use Coq to define simple data types.

    The [Inductive] command declares new data types; its syntax and meaning are
    similar to definitions of algebraic data types in languages such as Haskell
    and OCaml. Each declaration contains a list of values that specify how to
    form elements of that data type; each such value is called a
    _constructor_. For instance, here is how we define a data type [color]
    consisting of two elements: [Red] and [Black]. *)

Inductive color : Type :=
| Red
| Black.

(** The above definition adds three new constants to our environment: [color],
    [Red], and [Black]. Every valid expression in Coq has a type; we can ask Coq
    what the type of these three new constants is with the [Check] command. For
    instance: *)
...
(** It is usually preferable to avoid including [Check] commands in Coq files,
    ...
    Constructors of inductive data types can take arguments, which can include
    elements of the type being defined. The next declaration defines a red-black
    tree data type for storing number values; these are binary trees whose
    internal nodes are colored either [Red] or [Black], and contain an element
    of [nat], a standard Coq type representing natural numbers. *)

Inductive nat_tree : Type :=
| NatLeaf
| NatNode of color & nat_tree & nat & nat_tree.

(** Let's analyze this declaration. [NatLeaf] is a constructor that takes no
    arguments, as the [Red] and [Black] constructors of [color]. [Node], on the
    other hand, takes four arguments: a [color], a [nat_tree], a [nat], and
    another [nat_tree]. We can check that Coq understands our new definitions:
    *)

Check NatLeaf.
Check NatNode.

(** The arrows ([->]) indicate that [NatNode] expects arguments; each type to
    the left of an arrow specifies the type of an argument to [NatNode]. To use
    [NatNode], we must supply elements of matching types by writing them in
    sequence after that constructor: *)

Check NatNode Red NatLeaf 0 NatLeaf.
Check NatNode Black (NatNode Red NatLeaf 0 NatLeaf) 1 NatLeaf.

(** The first expression represents a tree of natural numbers that contains a
    single element: the number [0]. Its root node is colored [Red]. The second
    expression represents a tree of natural numbers that contains two elements,
    [0] and [1]. The number [1] is stored at the root of the tree, while [0] is
    stored on its left child. The root of that tree is colored [Black], while
    the subtree that stores [0] is colored [Red].

    As in other functional programming languages, we can make our definition
    more generic, by allowing our trees to store elements of any type [T] that
    we want: *)
...
(** Now, when we check the types of [Leaf] and [Node], we see that they are
    prefixed with a [forall], which allows us to choose what type the contents
    of our tree should have. *)

Check Leaf.
Check Node.

(** One thing that makes Coq different from other functional languages is that
    polymorphism is _explicit_: each use of a polymorphic value must be
    annotated with the type we want to instantiate it with, as if it were the
    argument to a function. For instance, this is how we use [Leaf] to construct
    an empty [tree] of [nat] elements: *)

Check Leaf nat.

(** Providing these annotations can be cumbersome, so Coq provides a
    shorthand. We can specify that the value of [T] should be inferred every
    time [Leaf] is used. This feature is known as _implicit arguments_. *)

Arguments Leaf {T}.

(** The curly braces around [T] in the above command declare that type as being
    implicit. In some cases, type parameters can be deduced from the type of the
    arguments of a term. For instance, the third argument of the [Node]
    constructor has exactly the type that is used as a parameter to
    [Node]. Hence, if we know the type of that argument, we know what [T] should
    be. The [Set Implicit Arguments] directive we used earlier instructs Coq to
    always declare such arguments as implicit. This allows us to write
    expressions such as the following: *)

Check Node Red Leaf 0 Leaf.
Check Node Black (Node Red Leaf 0 Leaf) 1 Leaf.

(** We can also turn off implicit arguments temporarily by prefixing an
    identifier with [@]. This is useful in situations where Coq doesn't have
    enough information to determine what the value of some type parameter should
    be. *)

Check @Node nat Red Leaf 0 (@Leaf nat).

(** Before we move on, a small comment: the [of] and [&] symbols are part of
    ssreflect. Here is what the previous declaration would look like in usual
    Coq syntax: *)
...
(** We have replaced the type and constructor names above to avoid clashes, but
    the two declarations are otherwise equivalent.


    * Definitions

    We can give names to values and functions with the [Definition] command. For
    instance: *)

Definition singleton (T : Type) (x : T) : tree T :=
  Node Red Leaf x Leaf.

(** The above declaration defines a function [singleton] that takes one
    argument, an element [x] of some type [T], which can be chosen
    arbitrarily. Its result is a [tree T] consisting of one internal node
    colored [Red] that contains [x]. We can see that this is reflected in the
    type of [singleton]: *)

Check singleton.

(** The syntax for applying functions is the same as for data constructors: *)

Check singleton 0.

(** Note that the above definition has type annotations in them. We can usually
    omit some or all of them, since Coq can infer them most of the time. Here,
    for instance, we don't annotate the result type of [three_times], and omit
    the type of [T]: *)

Definition three_times T (x : T) :=
  Node Black (singleton x) x (singleton x).

(** Nevertheless, it is often a good idea to include type annotations, since
    they can improve readability and serve as good documentation.

    We can use the [Compute] command to evaluate an expression: *)

Compute three_times 0.

(** We define recursive functions using the [Fixpoint] command. For instance,
    here is a function [tsize] that computes the number of elements stored in a
    tree: *)
...
(** Notice that this definition uses the addition operation on natural numbers,
    [+]. The [ssrnat] library of ssreflect comes with many other predefined
    functions on [nat], including multiplication, subtraction, etc. The [nat]
    type is used extensively in Coq developments, and we will encounter it
    often.
